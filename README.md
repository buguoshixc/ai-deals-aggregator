...
| 2 | `未标注截止日期` | 没有 `expiresAt`，`validity` 也没写长期 | 之后，按最近更新降序 |
| 3 | `长期活动` | 没有 `expiresAt`，`validity` 命中「长期 / 永久有效 / ongoing…」 | 最后，按最近更新降序 |
...
`npm run selftest:expiry` 验三件事：抽取器的正/负样例、前端三分类与排序次序、
以及前后端「长期」词表一致（两边只改一处会让角标与统计打架）。
`npm run validate` 会打印当前分布（有截止日期 / 未标注 / 长期活动）。
