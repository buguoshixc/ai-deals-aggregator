...
| E2 圆角 3 档 | ⚠️ 部分 | 卡片 10px / 控件 6px / 弹层 12px 已统一；`.lg` 的 7px、`.tg` 的 4px 等散值**未全部收敛**（低优先，未做） |
| E3 间距 4 的倍数 | ⚠️ 部分 | 新增样式（列表行、详情页、锚点条）都走 4 的倍数；老样式的 `13px 14px`、`9px` **未回改**（避免在定高卡片里引发布局回归） |
...
| S3 键盘可达 | ⚠️ 部分 | `:focus-visible` 与 Esc 关闭弹层已有；**「焦点归还触发元素」未单独断言**（弹层关闭后焦点行为未测） |
...
3. **E2/E3/S3/P4 只做了部分**：散值圆角与旧间距未全量收敛（避免在定高卡片里引发布局回归）；
   「弹层关闭后焦点归还」没有单独断言；英文页属 C 未做。
