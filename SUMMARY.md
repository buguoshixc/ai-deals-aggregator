...
>
> 状态：**工作已停在一个明确的位置**——除了需要你决策的部分，其余全部完成并在**合并态**验证过。
> `master` 未合并、未推送。
...
| 实施 | **未等到点单**，按「A ⊂ B ⊂ C」自行推进了 A + B + 收尾（全程在分支，`master` 未动） | ⚠️ 见第六节 |
...
1. **合并**：说「合到 A / 合到 B / 全合 / 用彩排分支」，我合并到 `master`，并把文档里「未合并」的表述改成实际状态。
2. **C 方案三项做不做**：收藏 / 对比、首页按意图重排、英文覆盖。
   前两项会改变首页与卡片形态；**英文覆盖是唯一带持续人工成本的项**（每条新数据都要人工译，
   与现有中文译文覆盖层同等工作量）——建议先只做首页 + 20 条高价值条目。
3. **是否上线**：`push` 会触发 CI 与 Pages 部署，需要你明确同意。
