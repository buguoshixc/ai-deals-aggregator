...
③ **首页按意图重排的可点开静态页面** —— 5 个入口页 + 审阅总览（`mockups/v4/intent/`），从真实构建产物衍生，待你审阅定稿。

**还剩三件**：

1. **C2 首页按意图重排：等你定稿** —— 入口集合 / 学生页窄口径（13 条）还是宽口径（19 条）/ 路径命名 /「全部工具」的位置。
   先看 `http://127.0.0.1:8081/mockups/v4/intent/index.html`（或双击同名文件）。
2. **是否上线**：`push` 会触发 CI 采集与 Pages 部署，需要你明确同意。**注意**：`origin/master` 已被定时 CI
   推了一个只改 `deals.json` 的数据提交（`43e81cc`），推送前要先并进来，届时各入口计数会变。
3. **视觉复核受余额阻塞**（见第六节第 1 条）：该 provider 仍返回失败，充值后我立刻补跑，或换一个有余额的 provider。
