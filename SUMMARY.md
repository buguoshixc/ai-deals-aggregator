...
**已办**：① **合并** —— 用户选「全合」，`git merge --no-ff trial/merge-rehearsal-2` 零冲突落地（`10cc923`）。
文档里「未合并」的表述已全部改为实际状态（本文件 + `PROJECT_STATUS.md` 2.21 + `docs/DESIGN-RULES.md`
+ `research/GAP-MATRIX.md`），并补上了原先缺失的 2.21 记录。

**还剩三件**：

1. **C 两项已定，按此推进**：做「收藏 / 对比」与「首页按意图重排」；**英文覆盖不做**（它是唯一带持续人工成本的项）。
   首页重排动手前会先出 mockup 给你定稿。
2. **是否上线**：`push` 会触发 CI 采集与 GitHub Pages 部署，需要你明确同意；本地预览已可看
   （`node scripts/serve.js --dir=dist` → `http://127.0.0.1:8080/`）。
